        ...
        if ($pendingOrder) {
            ...
            if ($recipe) {
                ...
                if ($ingredients->isNotEmpty()) {
                    ...
                    ])->post('gateway/api/warehouse/validate', [
                        'ingredients' => [
                            ['id' => 7, 'quantity' => '3']
                        ]
                    ])->json());
                    // dd($response = Http::post('gateway/api/warehouse/validate', [
                    //     'ingredients' => [
                    //         ['id' => 7, 'quantity' => '3']
                    //     ]
                    // ]));
                    //dd( $response = Http::post('gateway/api/warehouse/validate', ['ingredients' => $ingredientList]));
                    // $response = Http::post('http://gateway/api/warehouse/validate', [
                   ...
                   // dd($response);
                     //$response = Http::withHeaders(['Content-Type' => 'application/json', ])->post('gateway/api/warehouse/validate', ['data' => $jsonData]);
                    // Verificar si la solicitud fue exitosa
                    // $response = Http::withHeaders([
                    //     'Content-Type' => 'application/json',
                    // ])->post('http://gateway/api/warehouse/validate', ['ingredients' => $ingredientList]);
